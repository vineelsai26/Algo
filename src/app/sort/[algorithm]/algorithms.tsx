...
export async function insertionSort(bars: HTMLCollectionOf<HTMLElement>) {
	...
	for (i = 1; i < bars.length; i++) {
		...
		while (
			j >= 0 &&
			parseInt(bars[j].style.height.replace('px', '')) > temp
		) {
			bars[j].style.backgroundColor = 'red'
			bars[j + 1].style.backgroundColor = 'red'
...
export async function selectionSort(bars: HTMLCollectionOf<HTMLElement>) {
	...
	for (i = 0; i < bars.length - 1; i++) {
		...
		for (j = i + 1; j < bars.length; j++) {
			if (
				parseInt(bars[j].style.height.replace('px', '')) <
				parseInt(bars[min].style.height.replace('px', ''))
			) {
				min = j
...
async function heapify(
	...
	if (
		l < n &&
		parseInt(bars[l].style.height.replace('px', '')) >
			parseInt(bars[largest].style.height.replace('px', ''))
	) {
		largest = l
	}
	...
	if (
		r < n &&
		parseInt(bars[r].style.height.replace('px', '')) >
			parseInt(bars[largest].style.height.replace('px', ''))
	) {
		largest = r
	}
...
export async function quickSort(
	...
	while (right >= left) {
		if (
			parseInt(bars[right].style.height.replace('px', '')) <
				parseInt(bars[pivot].style.height.replace('px', '')) &&
			parseInt(bars[left].style.height.replace('px', '')) >
				parseInt(bars[pivot].style.height.replace('px', ''))
		) {
